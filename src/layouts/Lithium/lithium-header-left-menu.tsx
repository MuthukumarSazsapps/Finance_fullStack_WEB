...
import { DropdownItemType, MenuItemsType } from './lithium-menu-items';
    ...
    <motion.ul
      ...
      {items.map((item, index) => {
        if (item.subMenuItems) {
          const isActive = item.subMenuItems.some(subItem => subItem.href === pathname);
          return (
            <li
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                {
                  'bg-gray-100 text-gray-900 dark:bg-gray-50/50': pathname === item.href,
                },
              )}>
              {item.icon}
  ...
  (
    ...
    return (
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, transition: { duration: 0.2 } }}
        className={cn('flex gap-x-2 rounded-md border border-gray-300 p-1', className)}>
  ...
  return (
    <motion.ul
      ...
      {items.map((item, index) => {
        ...
        return (
          <li key={`link-menu-${item.name}-${index}`} className="relative my-0.5 ">
            <Link
              to={item.href ?? '/'}
              className={cn(
